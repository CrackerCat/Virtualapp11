...
public class InstrumentationDelegate extends Instrumentation {
    ...
    public InstrumentationDelegate(Instrumentation base) {
        fixInstrumentation(base);
        this.base = base;
        this.root = base;
    }

    private void fixInstrumentation(Instrumentation base) {
        try {
            Class<?> forName = Class.forName("android.app.Instrumentation");
            Field[] declaredFields = forName.getDeclaredFields();
            Field mThread = null;
            for (Field field:declaredFields){
                if (field.getName().equals("mThread")){
                    mThread = field;
                    mThread.setAccessible(true);
                }
            }
            if (mThread != null){
                Object activityThread = mThread.get(base);
                DebugKook.d("修復 Instrumentation 對象中 ActivityThread 為null 的情況 "+activityThread);
                if (activityThread == null){
                    mThread.set(base, ActivityThread.currentActivityThread.call());
                }
            }else {
                DebugKook.d("修復 Instrumentation 對象中 沒有 activityThread 屬性");
            }
        } catch (Exception e) {
