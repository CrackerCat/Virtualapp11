...
public final class SpecialComponentList {
    ...
    public static String getProtectActionPrefix() {
        if (PROTECT_ACTION_PREFIX == null) {
            PROTECT_ACTION_PREFIX = STATIC_PROTECT_ACTION_PREFIX + (VirtualCore.get().isMainProcess() ? 0 : VClient.get().getVUserHandle());
        }
    ...
    public static String protectAction(String originAction) {
        ...
        if (originAction.startsWith(getProtectActionPrefix())) {
            return originAction;
        }
        String newAction = PROTECTED_ACTION_MAP.get(originAction);
        if (newAction == null) {
            newAction = getProtectActionPrefix() + (VirtualCore.get().isMainProcess() ? 0 : VClient.get().getVUserHandle()) + originAction;
        }
        return newAction;
    }
    ...
    public static String unprotectAction(String action) {
        ...
        if (action.startsWith(getProtectActionPrefix())) {
