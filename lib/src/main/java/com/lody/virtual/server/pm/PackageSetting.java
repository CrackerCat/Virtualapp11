...
public class PackageSetting implements Parcelable {
    ...
    public String getApkPath(boolean is64bit) {
        if (appMode == InstalledAppInfo.MODE_APP_USE_OUTSIDE_APK) {
            try {
...
//        return new InstalledAppInfo(packageName, appMode, flag, appId);
        return new InstalledAppInfo(packageName, appMode, apkPath, libPath, dependSystem, appId, PackageCacheManager.get(packageName).xposedModule);
    ...
    public boolean isRunOn64BitProcess() {
        switch (flag) {
            ...
            case FLAG_RUN_BOTH_32BIT_64BIT:
                return !RUN_WITH_32BIT_FIRST && VirtualCore.getConfig().get64bitEnginePackageName() != null;
            default:
                return false;
        }
    }
